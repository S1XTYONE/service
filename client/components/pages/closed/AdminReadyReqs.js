...
export default class AdminReadyReqs extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount()
  {
    getInformationAboutApps('http://localhost:3001/doneapp')
    .then((response)=>{
  ...
  createTableSentStatus=()=>{
    ...
    for ( let i=0;i<this.state.dlina;i++)
    {
      ...
      table.push(
        ...
        <td><Button  onClick={this.handleClick} id = {field.applicationID}>Выдано</Button></td>
      </tr>
      )
    }
    return table;
